...
server.tool("register-notification", "Create a price alert for a specific token", {
    ...
    targetPrice: z
        .string()
        .describe("Target price at which the user wants to be alerted (e.g., '1928.23')"),
...
server.tool("get-user-notifications", "Fetch all price alerts for a specific user session", {
    session_id: z
        .string()
        .describe("Session ID used to filter user notifications"),
}, async ({ session_id }) => {
    try {
        const response = await fetch(`http://localhost:3001/api/userNotification?session_id=${session_id}`);
        ...
        const formatted = notifications
            .map((n, i) => `#${i + 1} - Token: ${n.token}, Target: ${n.targetPrice}, Condition: ${n.condition}`)
            .join("\n");
        return {
            content: [
                {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `❌ Failed to fetch notifications: ${error.message || "Unknown error"}`,
                },
            ],
        };
    }
});
server.tool("get-latest-token-price", "Retrieve the latest token prices from the notification server. This tool fetches and returns the current prices of various tokens as maintained by the system.", {}, // No input parameters required
async () => {
    try {
        const response = await fetch("http://localhost:3001/api/latestTokenPrice", {
            method: "GET",
            headers: { "Content-Type": "application/json" },
        });
        if (!response.ok) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Latest Token Prices:\n\n${JSON.stringify(data.latestTokenPrices, null, 2)}`,
                },
            ],
        };
    }
